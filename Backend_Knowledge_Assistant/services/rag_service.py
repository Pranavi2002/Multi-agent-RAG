...
# includes observability metrics and logging
# includes optional agents (router + translator)
...
# OPTIONAL AGENTS (NEW)
# ==============================
from agents.llm_router_agent import choose_llm
from agents.translator_agent import translate_if_needed
...
class RAGService:
    ...
    # ----------------------------
    # QUERY PIPELINE (RAG + OBSERVABILITY + OPTIONAL AGENTS)
    # ----------------------------
    def query(self, user_query: str, db):
        ...
        try:
            # =============================
            # 0️⃣ TRANSLATION AGENT
            # =============================
            normalized_query = translate_if_needed(user_query)
            ...
            # =============================
            # 5️⃣ LLM ROUTER AGENT
            # =============================
            selected_llm = choose_llm(user_query)

            if selected_llm == "fast":
                trimmed_context = context[:500]
                answer = self.llm.generate_answer(user_query, trimmed_context)
            else:
            ...
            logger.info(
                ...
                extra={
                    ...
                    "llm_used": selected_llm if 'selected_llm' in locals() else None
                }
            )
            ...
            try:
                save_query(
                    ...
                    avg_similarity=eval_score.get("avg_similarity", 0.0)
                    if 'eval_score' in locals() else 0.0,
                    max_similarity=eval_score.get("max_similarity", 0.0)
                    if 'eval_score' in locals() else 0.0,
                    latency=latency
                )
